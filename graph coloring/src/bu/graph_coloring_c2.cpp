...
OneMove* GraphColoring::findBestOneMove(int** coloradj, int** tabulist, OneMove* move, int niter, int nc, int bestnc)
{
	...
	OneMove besttabumove;

	bool choose = false;
	bool choose_tabu = false;
		...
		if (coloradj[cur->id-1][cur->color] > 0)
		{
			for (i = 0; i < numColors; ++i)
			{
				if (i != cur->color)
				{
					...
					{
						if (profit < best_tabu && profit<0)
						{
							choose_tabu = true;
							best_tabu = profit;
							besttabumove.id = cur->id;
							besttabumove.color = cur->color;
							besttabumove.bestnew = i;
						}
					}
				}
			}
	...
	if (choose_tabu)
	{
		if (choose)
		{
			if (best_tabu < best)
			{
				move->id = besttabumove.id;
				move->color = besttabumove.color;
				move->bestnew = besttabumove.bestnew;
			}
			else
			{
				move->id = bestmove.id;
